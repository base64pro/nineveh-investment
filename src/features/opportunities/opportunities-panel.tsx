...
export function OpportunitiesPanel() {
  ...
  const filtered = useMemo(() => {
    ...
    const min = areaMin.trim() === "" ? null : Number(areaMin);
    const max = areaMax.trim() === "" ? null : Number(areaMax);
    return all.filter((o) => {
      ...
      if (min !== null && (o.area_total_m2 ?? -Infinity) < min) return false;
      if (max !== null && (o.area_total_m2 ?? Infinity) > max) return false;
      if (needle) {
  ...
  }, [all, q, sector, district, oppStatus, availableOnly, areaMin, areaMax]);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="space-y-2 border-b border-border p-3">
        ...
        />
        {/* فلترة متقدّمة (§ب.2.6): القطاع · القضاء · الحالة · المساحة · متاحة */}
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          ...
          <input value={areaMin} onChange={(e) => setAreaMin(e.target.value)} type="number" placeholder="مساحة من" className="w-24 rounded-md border border-input bg-background px-2 py-1" />
          <input value={areaMax} onChange={(e) => setAreaMax(e.target.value)} type="number" placeholder="إلى" className="w-20 rounded-md border border-input bg-background px-2 py-1" />
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      {isAvailable(o) ? (
                        <span className="rounded-full bg-state-completed/15 px-2 py-0.5 text-[10px] text-state-completed ring-1 ring-state-completed/40">
                          متاحة
                        </span>
                      ) : null}
                  ...
                  <div className="mt-2 flex gap-1.5">
                    ...
                    <Button size="sm" variant="danger" onClick={() => void onDelete(o)}>
