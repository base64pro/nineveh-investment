...
// م5.3 → م7.5 · رسوم التقارير (Recharts) بثيمة هولوكرامية: تدرّجات + توهّج + حركات دخول. النقر ينتقل/يُصفّي.
...
/** غلاف توهّج موحّد للرسوم (drop-shadow هولوكرامي خفيف). */
function Glow({ children }: { children: ReactNode }) {
  return <div className="drop-shadow-[0_0_16px_rgba(148,175,209,0.22)]">{children}</div>;
}
  ...
  return (
    <Glow>
      <ResponsiveContainer width="100%" height={210}>
        <PieChart>
          <Pie data={data} dataKey="count" nameKey="label" innerRadius={46} outerRadius={78} paddingAngle={2.5} animationDuration={900} onClick={(d) => { const s = pick(d, "state"); if (s) onSlice?.(s); }}>
            {data.map((d) => <Cell key={d.state} fill={d.color} stroke="rgba(11,18,32,0.6)" strokeWidth={1.5} className="cursor-pointer outline-none" />)}
          </Pie>
    ...
    <Glow>
      <ResponsiveContainer width="100%" height={Math.min(360, Math.max(120, data.length * 30 + 24))}>
        <BarChart data={data} layout="vertical" margin={{ left: 4, right: 20, top: 4, bottom: 4 }}>
          ...
          <Bar dataKey="count" fill={`url(#${gid})`} radius={[0, 6, 6, 0]} maxBarSize={22} animationDuration={800} onClick={(d) => { const l = pick(d, "label"); if (l) onPick?.(l); }} className="cursor-pointer" />
        </BarChart>
      </ResponsiveContainer>
    </Glow>
  );
    ...
    <Glow>
      <ResponsiveContainer width="100%" height={210}>
        <LineChart data={data} margin={{ left: 4, right: 16, top: 8, bottom: 4 }}>
          ...
          <Tooltip contentStyle={TOOLTIP} />
          <Legend wrapperStyle={{ fontSize: 11, color: "#e8edf5" }} />
          <Line type="monotone" dataKey="opportunities" name="فرص" stroke="#C7A24E" strokeWidth={2.5} dot={{ r: 3 }} animationDuration={900} />
          <Line type="monotone" dataKey="licenses" name="رخص" stroke="#7d9cd1" strokeWidth={2.5} dot={{ r: 3 }} animationDuration={900} />
        </LineChart>
      </ResponsiveContainer>
    </Glow>
  );
