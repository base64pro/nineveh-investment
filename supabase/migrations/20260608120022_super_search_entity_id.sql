...
-- مراجعة م6 · إصلاح: البحث يعيد entity_id — النتيجة غير المرسومة تفتح **سجلّها** لا القسم فقط (§هـ.2.ج «فتح بياناته»).
...
create function super_search(
  ...
  parcel_no text, map_ref text, entity_id text, has_geom boolean, rank int
  ...
  select 'assumed', a.name, a.sector, a.district, a.state::text, a.parcel_no,
         a.id::text, a.id::text, (a.geom is not null),
...
comment on function super_search(text, text, text, text[], int) is 'م5.2 · بحث فائق حتمي مُطبَّع عربياً + مرجع خريطة + معرّف كيان لفتح السجلّ (لا تأليف).';
